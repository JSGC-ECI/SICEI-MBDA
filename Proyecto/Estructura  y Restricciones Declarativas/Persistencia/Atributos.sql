...
ALTER TABLE MATERIAS ADD CONSTRAINT chk_TIdMateriaMaterias
    CHECK (REGEXP_LIKE(idMateria, '^[A-Z]{4}[0-9]*$'));
...
ALTER TABLE ESTUDIANTES ADD CONSTRAINT chk_TIdCancelacionEstudiantes
    CHECK (REGEXP_LIKE(idCancelacion, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE MATERIASPORESTUDIANTE ADD CONSTRAINT chk_TIdEstudiante
    CHECK (REGEXP_LIKE(idEstudiante, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE MATERIASPORESTUDIANTE ADD CONSTRAINT chk_TIdMateriasPorEstudiante
    CHECK (REGEXP_LIKE(idMateria, '^[A-Z]{4}[0-9]*$'));
...
ALTER TABLE PREINSCRIPCIONES ADD CONSTRAINT chk_TEstadoPreinscripcion
    CHECK (estado IN ('Aceptada', 'Rechazada', 'Pendiente'));

ALTER TABLE PREINSCRIPCIONES ADD CONSTRAINT chk_TIdEstudiantePreinscripcion
    CHECK (REGEXP_LIKE(idEstudiante, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE PREINSCRIPCIONES ADD CONSTRAINT chk_TIdMateriaPreinscripcion
    CHECK (REGEXP_LIKE(idMateria, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE AREAS ADD CONSTRAINT chk_TIdAreas
    CHECK (REGEXP_LIKE(idArea, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE AREAS ADD CONSTRAINT chk_TIdDepartamentoAreas
    CHECK (REGEXP_LIKE(idDepartamento, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE AREAS ADD CONSTRAINT chk_TIDirectorAreas
    CHECK (REGEXP_LIKE(IDDIRECTOR, '^[A-Z]{4}[0-9]*$'));

ALTER TABLE DIRECTORES ADD CONSTRAINT chk_TMoneda
    CHECK (presupuestoAsignados BETWEEN 1 AND 99999999);

ALTER TABLE DIRECTORES ADD CONSTRAINT chk_TMonedaDirectores
    CHECK (PRESUPUESTOASIGNADOS BETWEEN 1 AND 99999999);
